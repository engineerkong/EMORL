...
class MetaLearner(nn.Module):
    ...
    def __call__(self, input_ids=None, attention_mask=None, labels=None, output_hidden_states=False, return_dict=True, **kwargs):
        ...
        # # If labels are provided, calculate loss (similar to what the base model would do)
        # loss = None
        # if labels is not None:
        #     # Use the same loss calculation as the base model
    ...
    def forward(self, gen_input, num_runs, train_batch_size):
        ...
        generated_tokens = torch.zeros(train_batch_size*num_runs, 32, dtype=torch.long, device=self.device)
        # Generation by step
...
def meta_train(meta_model, tokenizer, train_dataloader, val_dataloader, optimizer, scaler, scorer, rl_crit, gens_params, 
    ...
    while step_count < num_steps and training:
        ...
        for batch in train_dataloader:
            ...
            meta_model.train()
            generated_tokens = meta_model.forward(gen_input, num_runs, train_batch_size)
